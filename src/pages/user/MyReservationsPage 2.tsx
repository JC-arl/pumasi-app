...
export default function MyReservationsPage() {
  ...
  const [selectedTab, setSelectedTab] = useState<'applied' | 'cancelled' | 'completed'>('applied');
  ...
  const filteredCombinedReservations = useMemo(() => {
    const filtered = combinedReservations.filter(reservation => {
      if (selectedTab === 'applied') {
        return ['PENDING', 'APPROVED', 'CHECKED_OUT', 'PENDING_RETURN'].includes(reservation.status);
      }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-6">
        <nav className="flex space-x-8">
          ...
          >
            신청 내역 ({combinedReservations.filter(r => ['PENDING', 'APPROVED', 'CHECKED_OUT', 'PENDING_RETURN'].includes(r.status)).length})
          </button>
          <button
          ...
          <button
            ...
            style={selectedTab === 'completed' ? {
              borderBottomColor: '#133E87',
              color: '#133E87'
            } : {}}
          >
            완료됨 ({combinedReservations.filter(r => r.status === 'RETURNED').length})
          </button>
        </nav>
      </div>
