...
export default function AssetDetailPage() {
  ...
  const handleReservation = (specification: MachinerySpecification) => {
    ...
    const confirmReservation = confirm(
      `${machinery.name} (${specification.specification})을 예약하시겠습니까?\n\n` +
      `제조사: ${specification.manufacturer}\n` +
      `임대료: ₩${specification.rentalPrice.toLocaleString()}/일`
    );

    if (confirmReservation) {
      const result = createReservation(machinery, specification, office?.name || '알 수 없음');
      
      if (result.success) {
        // 예약 완료 알림 생성
        createReservationNotification('confirmed', machinery.name, result.reservationId!);
        
        alert(`예약이 완료되었습니다!\n예약번호: ${result.reservationId}`);
        // 페이지 새로고침으로 업데이트된 재고 반영
